<template>
  <b-overlay class="body-con" :show="showLoading">
    ...
    <div class="main-con" style="background-color: black; width: 100%; height: 100%" :aria-hidden="showLoading ? 'true' : null">
      <b-alert
          v-model="showBottom"
                ...
                <div class="row justify-content-center my-auto">
                    <div class="col-md-8 col-10 my-5">
                      ...
                      <div class="align-items-center row mb-5">
                        ...
                        <h6 class="msg-info">Password reset</h6>
                      <form v-on:submit.prevent="submitForm">
                        <div class="form-group"> <label class="form-control-label text-muted">New Password</label> <input type="password" id="new_password" placeholder="New Password" class="form-control" v-model="new_password" required> </div>
                        <div class="form-group"> <label class="form-control-label text-muted">Password Confirm</label> <input type="password" id="confirm_password" placeholder="Confirm Password" class="form-control" v-model="confirm_password" required> </div>
                        <div class="row justify-content-center my-3 px-3"> <button class="btn-block btn-color" type="submit">Login</button> </div>
                      </form>
                      <div class="row justify-content-center my-2"> <button class="bg-transparent border-0" @click="forgotPasswordPage = true"><small class="text-muted">Forgot Password?</small></button> </div>
                    </div>
                </div>
                <div class="bottom text-center mb-5">
                    <p href="#" class="sm-text mx-auto mb-3">Don't have an account?<button class="btn btn-white ml-2" onclick="window.location.href='/signUp'">Create new</button></p>
                </div>
            </div>
        </div>
    </div>
  ...
  data(){
    ...
    submitForm(){
      ...
      if (this.new_password === this.confirm_password){
        ...
        axios.post('api/v1/users/reset_password_confirm/', formData)
          .then(response => {
          ...
          .catch(error => {
            console.log(error)
            this.showLoading = false
            this.reportMessage = error.response.data
